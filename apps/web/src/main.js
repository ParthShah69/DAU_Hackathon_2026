...
function canManageListings(listing = null) {
  if (isDemoMode || state.live.status === 'fallback') return true
  const identity = state.data.identity
  const roles = identity?.capabilities || []
  const isSupplier = roles.some((role) => ['supplier_admin', 'supplier_editor', 'seller_editor'].includes(role))
  if (!isSupplier) return false
  return !listing || listing.raw?.supplierOrganizationId === identity?.currentOrganization?.id
}

function workspaceLabel() {
  return state.data.identity?.currentOrganization?.name || 'Aster Fermentation Works'
}

function actorLabel() {
  return state.data.identity?.user?.displayName || 'Ananya'
}
...
async function publishSelectedListing() {
  ...
  if (!item) return
  if (!canManageListings(item)) return setNotice('Only the supplier organization can publish this listing.')
...
async function editSelectedListing(form) {
  ...
  if (!canManageListings(item)) return setNotice('Only the supplier organization can edit this listing.')
...
function assistantPanel() {
  return `<aside class="assistant-panel"><div class="assistant-panel-header"><div class="assistant-title"><span class="assistant-spark">✦</span><div><strong>CarbonBridge assistant</strong><span><i></i> ${state.live.status === 'connected' ? 'Live API' : 'Demo'} · ${escapeHtml(workspaceLabel())} workspace</span></div></div><div><button class="icon-button subtle" data-view="assistant" aria-label="Open full assistant">↗</button><button class="icon-button subtle" data-action="close-assistant" aria-label="Close assistant">×</button></div></div><div class="assistant-context"><span>Context</span><button>${escapeHtml(actorLabel())}'s workspace <b>×</b></button></div>${messagesHtml(state.messages.slice(-5))}<div class="assistant-composer">${composer()}</div></aside>`
}

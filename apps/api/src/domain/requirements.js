...
function validDate(value, field) {
  ...
  if (Number.isNaN(parsed.getTime())) throw new DomainError('INVALID_DATE', `${field} must be a valid ISO date`);
...
function listRequirements(store, actorOrganizationId) {
  return store.findMany('requirements', (item) => item.organizationId === actorOrganizationId).map(structuredClone);
}
...
module.exports = { normalizeRequirement, createRequirement, listRequirements, getRequirement };
